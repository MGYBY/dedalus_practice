...
init_timestep = 0.20*0.50*(Lx/Nx)/gw_c
...
problem.add_equation("dt(h) + dx(q) = 0")
problem.add_equation("dt(q) + (1/ve_coeff)*dx((9*q*q)/(7*h)+(5)/(4*rey*tan_theta)*h*h) = (1/ve_coeff)*(q/(7*h)*dx(q) + 5/(2*film_param*rey)*(h-q/h/h) + 5*film_param*film_param*web/2/rey*h*dx(dx(dx(h))) + (film_param/rey)*(9/2*dx(dx(q)) - 9/(2*h)*dx(q)*dx(h) + (4*q)/(h*h)*(dx(h)**2) - (6*q)/h*dx(dx(h))) + ((5*deb*(1-lambda_r))/(2*rey*h**4))*(6*h*q*dx(q) - 6*q*q*dx(h)))")
